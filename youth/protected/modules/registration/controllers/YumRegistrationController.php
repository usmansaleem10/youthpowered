...
class YumRegistrationController extends YumController {
  ...
  public function actionRegistration() {
    ...
    Yii::import('usertype.models.*');



    $form = new YumRegistrationForm;
    $profile = new YumProfile;
    // Include UserType Model
    $userTypeModel = new YumUserType;
    $userTypeObj = $userTypeModel->findAll();
    ...
    if (isset($_POST['YumRegistrationForm'])) { 
      ...
      if(!$form->hasErrors() && !$profile->hasErrors()) {
        $user = new YumUser;
        $user->register($form->username, $form->password, $profile, $form->type_id);
