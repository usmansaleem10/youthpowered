<h3 class="row-title"> <?php echo CHtml::encode($data->title); ?> </h3> 
...
<?php if(isset($data->owner))
	...
	<?php echo CHtml::link(Yum::t('View Details'), array(
					'//usergroup/groups/view', 'id' => $data->id), array('class'=>' btn btn-primary')); ?>
	<?php 
	if(is_array($data->participants) &&
			in_array(Yii::app()->user->id, $data->participants))
	echo CHtml::link(Yum::t('Leave group'), array(
				'//usergroup/groups/leave', 'id' => $data->id), array('class'=>' btn btn-primary')); 
	else
	echo CHtml::link(Yum::t('Join group'), array(
				'//usergroup/groups/join', 'id' => $data->id), array('class'=>' btn btn-primary')); ?>
