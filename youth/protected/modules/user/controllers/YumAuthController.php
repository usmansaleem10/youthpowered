			...
			if ($success !== false && $success instanceof YumUser) {
				...
				if($login_type && $login_type != 'debug') 
					...
					$cookie->expire = time() + Yum::module()->cookieDuration;
                                        $cookieUser = new CHttpCookie('user_id', serialize($success->id));
                                              
					$cookieUser->expire = time() + Yum::module()->cookieDuration;
                                        Yii::app()->request->cookies['user_id'] = $cookieUser;
                                        Yii::app()->request->cookies['login_type'] = $cookie;
                                        // setting cookie user name By Rosheel
                                        $cookieUserName = new CHttpCookie('username', serialize($success->username));
					$cookieUserName->expire = time() + Yum::module()->cookieDuration;
...
//                                        $name=$session['username'];
                                        ...
                                        // setting Sessions
//                                        $userSessionId = new CHttpSession('user_id', serialize($success->id));
//                                        Yii::app()->request->session = $userSessionId;
//                                        print_r($session['username']);
//                                        exit('gggg');
                                        
                                        
                                       // Setting Session UserName and UserId    
                                        //echo "<pre>";
                                        //print_r($success->avatar);
                                        //exit;
                                        Yii::app()->session['username'] = $success->username;
                                        Yii::app()->session['user_id'] = $success->id;
                                        Yii::app()->session['avatar'] = $success->avatar;
                                    
                                        $_SESSION['username']= $success->username;
                                        $_SESSION['user_id'] = $success->id;
                                        $_SESSION['avatar'] = $success->avatar;
                                        
                                        
//                                        Yii::app()->session->add('session_name',$success->username);
//
//                                        print_r(Yii::app()->session->add('session_name',$success->username);
